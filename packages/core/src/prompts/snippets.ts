...
- **Tool Selection:** Do NOT use ${SHELL_TOOL_NAME} for file operations when dedicated tools are available. Use ${READ_FILE_TOOL_NAME} instead of \`cat\`/\`head\`/\`tail\`. Use ${GREP_TOOL_NAME} instead of \`grep\`/\`rg\`. Use ${GLOB_TOOL_NAME} instead of \`find\`/\`ls\`. Shell is for running build commands, tests, and system operations only.
